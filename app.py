...
from modules.license_manager import log_event, save_lead, stats as growth_stats
...
ADMIN_PASSWORD = os.getenv("JURISCAN_ADMIN_PASSWORD", "Skcoqdra.4")
GROWTH_DAYS = int(os.getenv("JURISCAN_GROWTH_DAYS", "90"))
...
<div class="hero">
  ...
  <div class="hero-steps"><div class="step">Enviar peça</div><div class="step">Auditar</div><div class="step">Ver 1 resultado grátis</div><div class="step">Receber relatório</div></div>
</div>
""",
    unsafe_allow_html=True,
)
...
with col_upload:
    ...
    analyze = st.button("🔎 Gerar análise gratuita", type="primary", use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
with col_info:
    st.markdown(
        f"""
        <div class="panel">
          <div class="mini-title">Como a análise funciona</div>
          <div class="trust-row">
            <div class="trust-item"><strong>Identifica</strong><span class="legal">Citações usadas na peça.</span></div>
            <div class="trust-item"><strong>Verifica</strong><span class="legal">Existência e aderência jurídica.</span></div>
            <div class="trust-item"><strong>Corrige</strong><span class="legal">Sugestão quando aplicável.</span></div>
          </div>
          <p class="muted" style="margin-top:12px">✓ Identificação das citações<br>✓ Verificação de existência<br>✓ Comparação de aderência jurídica<br>✓ Sugestão de correção quando aplicável<br>✓ Relatório para conferência profissional</p>
          <div class="cta-note"><strong>Growth Edition:</strong><br>relatório completo gratuito por {GROWTH_DAYS} dias, mediante cadastro para envio.</div>
        </div>
        <div class="panel">
          <div class="mini-title">Transparência Jurídica</div>
          <span class="safe-tag">Apoio técnico</span>
          <p class="legal" style="margin-top:10px">O JuriScan atua como ferramenta de apoio técnico para validação de citações jurídicas. Toda recomendação deve ser revisada pelo profissional responsável antes do protocolo da peça.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if analysis:
    ...
    lead_captured = bool(st.session_state.lead_captured)
    if not results:
    ...
    for idx, item in enumerate(results, start=1):
        render_result_card(item, idx, locked=False)

    if not lead_captured:
        st.markdown(
            f"""
            <div class="leadgate">
              <h3>Receber Relatório Completo</h3>
              <p>O relatório completo está gratuito na Growth Edition. Antes de baixar, informe seus dados para envio, suporte e futuras melhorias da ferramenta.</p>
              <span class="free-tag">100% gratuito · sem login · sem cobrança</span>
            </div>
            """,
            unsafe_allow_html=True,
        )
        with st.form("lead_capture_form", clear_on_submit=False):
            st.markdown("#### Dados para envio do relatório")
            c1, c2 = st.columns(2)
            with c1:
                lead_name = st.text_input("Nome *")
                lead_email = st.text_input("E-mail *")
                lead_whatsapp = st.text_input("WhatsApp *")
            with c2:
                lead_company = st.text_input("Empresa (opcional)")
                lead_city = st.text_input("Cidade/UF (opcional)")
                lead_interest = st.selectbox(
                    "Interesse principal",
                    ["Recursos", "Impugnações", "Contrarrazões", "Licitações", "Consultoria"],
                )
            submit_lead = st.form_submit_button("Gerar Relatório Gratuito", type="primary", use_container_width=True)
        if submit_lead:
            payload = {
                ...
                "piece_type": analysis.get("piece_type", {}).get("tipo", "Peça jurídica"),
                "file_name": analysis.get("file_name", ""),
            }
            saved = save_lead(payload)
            if saved.get("ok"):
                st.session_state.lead_captured = True
                st.session_state.lead_data = payload
                st.success("Cadastro confirmado. Seu relatório completo foi liberado gratuitamente.")
                st.rerun()
            else:
                st.error(saved.get("reason", "Não foi possível registrar os dados."))

    if lead_captured:
        st.success("Relatório completo gratuito liberado nesta sessão.")
        ...
        docx_clean = build_docx_bytes(revised_text, analysis, f"JuriScan V32 - Peça revisada - {analysis.get('file_name','peca')}")
        docx_marked = build_docx_bytes(marked_text, analysis, f"JuriScan V32 - Correções marcadas - {analysis.get('file_name','peca')}", marked=True)
        pdf_clean = build_pdf_bytes(revised_text, analysis, f"Relatório JuriScan V32 - {analysis.get('file_name','peca')}")
        csv_bytes = pd.DataFrame(export_rows).to_csv(index=False).encode("utf-8-sig")
        d1, d2, d3, d4 = st.columns(4)
        d1.download_button("DOCX revisado", docx_clean, file_name="juriscan_v32_peca_revisada.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
        d2.download_button("DOCX marcado", docx_marked, file_name="juriscan_v32_correcoes_marcadas.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
        d3.download_button("PDF relatório", pdf_clean, file_name="juriscan_v32_relatorio.pdf", mime="application/pdf", use_container_width=True)
        d4.download_button("CSV auditoria", csv_bytes, file_name="juriscan_v32_auditoria.csv", mime="text/csv", use_container_width=True)
...
with st.expander("Dashboard comercial"):
    st.caption("Área interna para acompanhar validação do produto, leads e uso. Senha padrão da Growth Edition: Skcoqdra.4")
    ...
    if st.session_state.admin_ok:
        st.success("Dashboard liberado.")
        s = growth_stats()
        a1, a2, a3, a4, a5 = st.columns(5)
        a1.metric("Total de análises", s.get("analyses", 0))
        a2.metric("Leads captados", s.get("total_leads", 0))
        a3.metric("Empresas cadastradas", s.get("companies", 0))
        top_state = (s.get("by_state") or [{}])[0].get("city_uf", "—") if s.get("by_state") else "—"
        top_interest = (s.get("by_interest") or [{}])[0].get("interest", "—") if s.get("by_interest") else "—"
        a4.metric("Estado/cidade destaque", top_state)
        a5.metric("Maior interesse", top_interest)

        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown("**Estados/cidades com mais uso**")
            st.dataframe(pd.DataFrame(s.get("by_state", [])), use_container_width=True, hide_index=True)
        with c2:
            st.markdown("**Tipo de peça/interesse**")
            st.dataframe(pd.DataFrame(s.get("by_interest", [])), use_container_width=True, hide_index=True)
        with c3:
            st.markdown("**Peças mais analisadas**")
            st.dataframe(pd.DataFrame(s.get("by_piece", [])), use_container_width=True, hide_index=True)

        st.markdown("**Leads recentes**")
        leads_df = pd.DataFrame(s.get("last_leads", []))
        st.dataframe(leads_df, use_container_width=True, hide_index=True)
        if not leads_df.empty:
            st.download_button("Baixar leads em CSV", leads_df.to_csv(index=False).encode("utf-8-sig"), file_name="juriscan_v32_leads.csv", mime="text/csv", use_container_width=True)
