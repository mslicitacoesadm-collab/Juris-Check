...
from modules.citation_extractor import classify_piece_type, detect_thesis, extract_references_with_context, parse_manual_query, split_into_argument_blocks
...
st.set_page_config(page_title='Atlas dos Acórdãos V19', page_icon='⚖️', layout='wide')
...
with c_hero:
    st.markdown(
        """
        <div class="hero">
          <h1>Atlas dos Acórdãos · V19 Precisão de Busca</h1>
          <p><strong>Auditoria de citações jurídicas com busca em camadas</strong>, parser de referência, validação contextual e reescrita jurídica mais segura.</p>
          <p>Esta versão fortalece o ponto crítico do produto: localizar o precedente certo quando o usuário informa número específico e, quando isso não for possível, entregar fallback por tese com explicação transparente.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
st.info('A busca manual agora tenta: referência exata → número normalizado → fallback por tese. Isso reduz falha quando o usuário digita um acórdão específico.')
...
with main_tab:
    ...
    if analysis:
        ...
        with t1:
            st.markdown(
                '<div class="block"><div class="small"><strong>Diagnóstico técnico:</strong> a peça foi lida como <strong>{}</strong>, com <strong>{}</strong> referências explícitas e <strong>{}</strong> blocos argumentativos passíveis de reforço. O motor passou a distinguir erro de número, citação válida com contexto fraco e substituição mais aderente.</div></div>'.format(
        ...
        with t2:
            ...
            for item in analysis['citation_results']:
                ...
                st.markdown(
                    f"<div class='card'><div style='display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:.55rem'><span class='legend' style='background:{bg};color:{fg}'>{item['status_label']}</span><span class='legend' style='background:{cbg};color:{cfg}'>{item['grau_confianca']}</span><span class='legend' style='background:#e8eef8;color:#123b67'>{item.get('camada_busca','Busca padrão')}</span></div><div class='small'><strong>Referência encontrada:</strong> {item['raw']}<br><strong>Linha:</strong> {item.get('linha','-')}<br><strong>Tese do contexto:</strong> {item.get('tese','Tese geral')}<br><strong>Classificação do erro:</strong> {item.get('tipo_erro','—')}</div></div>",
                    unsafe_allow_html=True,
                )
                ...
                if item.get('matched_record'):
                    ...
                    st.write(item['matched_record']['citacao_curta'])
        ...
        with t4:
            ...
            d1.download_button('DOCX limpo', docx_clean, file_name='atlas_v19_revisado_limpo.docx', mime='application/vnd.openxmlformats-officedocument.wordprocessingml.document', use_container_width=True)
            d2.download_button('DOCX marcado', docx_marked, file_name='atlas_v19_revisado_marcado.docx', mime='application/vnd.openxmlformats-officedocument.wordprocessingml.document', use_container_width=True)
            d3.download_button('PDF limpo', pdf_clean, file_name='atlas_v19_revisado_limpo.pdf', mime='application/pdf', use_container_width=True)
            d4.download_button('CSV da auditoria', csv_bytes, file_name='atlas_v19_auditoria.csv', mime='text/csv', use_container_width=True)

with manual_tab:
    st.markdown('<div class="section-title">Busca manual robusta por tese ou referência direta</div>', unsafe_allow_html=True)
    ...
    if st.button('Pesquisar precedentes', use_container_width=True):
        if not db_files:
            ...
            st.markdown(f"<div class='block'><div class='small'><strong>Modo detectado:</strong> {'referência específica' if result['search_mode']=='referencia_especifica' else 'busca por tese'}<br><strong>Leitura da consulta:</strong> tipo={parsed.get('kind')} | número={parsed.get('numero') or '—'} | ano={parsed.get('ano') or '—'} | colegiado={parsed.get('colegiado') or '—'}<br><strong>Camada dominante:</strong> {result['lookup_layer_label']}<br><strong>Tese inferida:</strong> {parsed.get('thesis_label')}</div></div>", unsafe_allow_html=True)
            if not result['matches']:
                st.info('Nenhum precedente relevante foi localizado com os filtros atuais. Revise o número informado ou tente complementar com a tese.')
            for rec in result['matches']:
                layer = rec.get('lookup_layer_label') or rec.get('lookup_layer') or 'Busca híbrida'
                st.markdown(f"<div class='card'><div style='display:flex;justify-content:space-between;gap:1rem;flex-wrap:wrap'><div><strong>{rec['citacao_curta']}</strong></div><div><span class='legend' style='background:#e8eef8;color:#123b67'>Aderência {int(rec['compat_score']*100)}%</span></div></div><div class='small' style='margin-top:.55rem'><strong>Tema:</strong> {rec.get('tema') or 'Não informado'}<br><strong>Camada:</strong> {layer}<br><strong>Motivo do match:</strong> {rec.get('motivo_match') or 'Sem explicação adicional.'}</div></div>", unsafe_allow_html=True)
