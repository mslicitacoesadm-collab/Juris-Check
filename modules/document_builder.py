...
def build_docx_bytes(revised_text: str, arg2: Any = None, arg3: Any = None, marked: bool = False) -> bytes:
    ...
    info = doc.add_paragraph()
    info.add_run('Auditoria JuriScan: ').bold = True
    info.add_run('validação de citações, comparação de precedentes, grau de confiança e revisão contextual.')
    if marked:
        p = doc.add_paragraph()
    ...
    for part in (revised_text or '').split('\n'):
        ...
        if marked and ('[AJUSTE JURISCAN]' in part or '[TRECHO AJUSTADO]' in part or '[[SUBSTITUIR POR:' in part):
